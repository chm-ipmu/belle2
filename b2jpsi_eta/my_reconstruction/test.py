...
def reconstruction(input_file, output_file):
    ...
    ma.inputMdst(environmentType="default", filename=input_file, path=mypath)

    ma.fillParticleList("e+:good", "", path=mypath)
    ma.fillParticleList("e-:good", "", path=mypath)
    ...
    ma.fillParticleList(
        decayString="gamma:all", cut="E < 1.0", writeOut=False, path=mypath
    )
    ...
    ma.looseMCTruth(list_name="e+:corrected", path=mypath)

    ## reconstruct J/psi -> e+ e- decay
    ma.reconstructDecay(
        "J/psi:ee -> e+:corrected e-:corrected",
        "2.8 < M < 3.2 and nTracks > 4 and useCMSFrame(p) < 2. ",
        path=mypath,
    )
    ma.reconstructDecay(
    ...
    # sc.stdK('good')
    ma.fillParticleList(
        decayString="pi+:good", cut="chiProb > 0.001 and pionID > 0.1", path=mypath
    )
    ma.fillParticleList(
        decayString="K+:good", cut="chiProb > 0.001 and kaonID > 0.1", path=mypath
    )

    ##psi(2S) my_reconstruction
    ma.reconstructDecay(
        decayString="psi(2S):ee -> J/psi:ee pi+:good pi-:good",
        cut="3.5 < M < 3.8 ",
        path=mypath,
    )

    ma.reconstructDecay(
        decayString="psi(2S):den -> e+:corrected e-:corrected",
        cut="3.6 < M < 3.8 and nTracks > 4 and useCMSFrame(p) < 3.",
        path=mypath,
    )

    ma.reconstructDecay(
        decayString="psi(2S):gen -> mu+:good mu-:good",
        cut="3.6 < M < 3.8 and nTracks > 4 and useCMSFrame(p) < 3.",
        path=mypath,
    )
    ...
    ma.reconstructDecay(
        decayString="psi(2S):mumu -> J/psi:mumu pi+:good pi-:good",
        cut="3.5 < M < 3.8 ",
        path=mypath,
    )
    ...
    ma.fillParticleList(
        decayString="K_S0:pipi -> pi+:good pi-:good", cut="0.3 < M < 0.7 ", path=mypath
    )
    ...
    ma.reconstructDecay(
        "B0:psi2s_eeks -> psi(2S):ee K_S0:pipi",
        "Mbc > 5.2 and abs(deltaE)<0.15 ",
        path=mypath,
    )
    ma.reconstructDecay(
        "B0:psi2s_mumuks -> psi(2S):mumu K_S0:pipi",
        "Mbc > 5.2 and abs(deltaE)<0.15 ",
        path=mypath,
    )
    ma.reconstructDecay(
        "B0:psi2s_denks -> psi(2S):den K_S0:pipi",
        "Mbc > 5.2 and abs(deltaE)<0.15",
        path=mypath,
    )
    ma.reconstructDecay(
        "B0:psi2s_genks -> psi(2S):gen K_S0:pipi",
        "Mbc > 5.2 and abs(deltaE)<0.15",
        path=mypath,
    )
    ...
    ma.reconstructDecay(
        "B+:psi2s_eekp -> psi(2S):ee K+:good",
        "Mbc > 5.2 and abs(deltaE) < 0.15 ",
        path=mypath,
    )
    ma.reconstructDecay(
        "B+:psi2s_mumukp -> psi(2S):mumu K+:good",
        "Mbc > 5.2 and abs(deltaE)< 0.15 ",
        path=mypath,
    )
    ma.reconstructDecay(
        "B+:psi2s_denkp -> psi(2S):den K+:good",
        "Mbc > 5.2 and abs(deltaE)<0.15",
        path=mypath,
    )
    ma.reconstructDecay(
        "B+:psi2s_genkp -> psi(2S):gen K+:good",
        "Mbc > 5.2 and abs(deltaE)<0.15",
        path=mypath,
    )
    ...
    vx.vertexRave(
        "B0:psi2s_eeks",
        0.0,
        "B0 -> [psi(2S):ee -> [J/psi:ee -> ^e+ ^e-] ^pi+ ^pi-] K_S0:pipi",
        "iptube",
        path=mypath,
    )
    vx.vertexRave(
        "B0:psi2s_mumuks",
        0.0,
        "B0 -> [psi(2S):mumu -> [J/psi:mumu -> ^mu+ ^mu-] ^pi+ ^pi-] K_S0:pipi",
        "iptube",
        path=mypath,
    )
    vx.vertexRave(
    ...
    vx.vertexRave(
        "B+:psi2s_eekp",
        0.0,
        "B+ -> [psi(2S):ee -> [J/psi:ee -> ^e+ ^e-] ^pi+ ^pi-] K+:good",
        "iptube",
        path=mypath,
    )
    vx.vertexRave(
        "B+:psi2s_mumukp",
        0.0,
        "B+ -> [psi(2S) -> [J/psi:mumu -> ^mu+ ^mu-] ^pi+ ^pi-] K+:good",
        "iptube",
        path=mypath,
    )
    vx.vertexRave(
    ...
    vx.vertexRave(
        "B+:psi2s_genkp",
        0.0,
        "B+ -> [psi(2S):mumu -> ^mu+ ^mu-] K+:good",
        "iptube",
        path=mypath,
    )

    ma.rankByHighest(
        "B0:psi2s_eeks", "chiProb", numBest=3, outputVariable="B_vtx_rank", path=mypath
    )
    ma.rankByHighest(
        "B0:psi2s_mumuks",
        "chiProb",
        numBest=3,
        outputVariable="B_vtx_rank",
        path=mypath,
    )
    ma.rankByHighest(
        "B0:psi2s_denks", "chiProb", numBest=3, outputVariable="B_vtx_rank", path=mypath
    )
    ma.rankByHighest(
        "B0:psi2s_genks", "chiProb", numBest=3, outputVariable="B_vtx_rank", path=mypath
    )

    ma.rankByHighest(
        "B+:psi2s_eekp", "chiProb", numBest=3, outputVariable="B_vtx_rank", path=mypath
    )
    ma.rankByHighest(
        "B+:psi2s_mumukp",
        "chiProb",
    ...
    ma.rankByHighest(
        "B+:psi2s_denkp", "chiProb", numBest=3, outputVariable="B_vtx_rank", path=mypath
    )
    ma.rankByHighest(
        "B+:psi2s_genkp", "chiProb", numBest=3, outputVariable="B_vtx_rank", path=mypath
    )
    ...
    ft.flavorTagger(
        particleLists=["B0:psi2s_eeks"], weightFiles="B2JpsiKs_muBGx1", path=mypath
    )
    ft.flavorTagger(
        particleLists=["B0:psi2s_mumuks"], weightFiles="B2JpsiKs_muBGx1", path=mypath
    )
    ft.flavorTagger(
        particleLists=["B0:psi2s_denks"], weightFiles="B2JpsiKs_muBGx1", path=mypath
    )
    ft.flavorTagger(
        particleLists=["B0:psi2s_genks"], weightFiles="B2JpsiKs_muBGx1", path=mypath
    )

    ft.flavorTagger(
        particleLists=["B+:psi2s_eekp"], weightFiles="B2JpsiKs_muBGx1", path=mypath
    )
    ft.flavorTagger(
        particleLists=["B+:psi2s_mumukp"], weightFiles="B2JpsiKs_muBGx1", path=mypath
    )
    ft.flavorTagger(
        particleLists=["B+:psi2s_denkp"], weightFiles="B2JpsiKs_muBGx1", path=mypath
    )
    ft.flavorTagger(
        particleLists=["B+:psi2s_genkp"], weightFiles="B2JpsiKs_muBGx1", path=mypath
    )
    ...
    ma.variablesToNtuple(
        "B0:psi2s_eeks", B0e_vars, filename=output_file, treename="b0e", path=mypath
    )
    ma.variablesToNtuple(
        "B0:psi2s_mumuks", B0m_vars, filename=output_file, treename="b0m", path=mypath
    )
    ma.variablesToNtuple(
        "B0:psi2s_denks", B0e_vars, filename=output_file, treename="b0den", path=mypath
    )
    ma.variablesToNtuple(
        "B0:psi2s_genks", B0m_vars, filename=output_file, treename="b0gen", path=mypath
    )
