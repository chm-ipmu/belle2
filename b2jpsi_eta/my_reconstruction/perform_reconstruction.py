...
class Reconstruction:
    ...
    def fill_particle_lists(self) -> None:
        ...
        # J/psi decays
        if self.has_jpsi2ee:
            ma.fillParticleList('e+', 'electronID > 0.5 and abs(d0) < 1 and abs(z0) < 4', path=self.path)
        elif self.has_jpsi2mumu:
            ma.fillParticleList('mu+', 'muonID > 0.5 and abs(d0) < 1 and abs(z0) < 3', path=self.path)

        # eta decays
        if self.has_eta2pipigamma:
            ma.fillParticleList('pi+', '', path=self.path)
    ...
    def reconstruction(self, input_file: str, output_file: str) -> None:
        ...
        ma.variablesToNtuple('J/psi', variables, filename=output_file, treename="jpsi", path=self.path)
        ma.variablesToNtuple('eta', variables, filename=output_file, treename="eta", path=self.path)
        ma.variablesToNtuple('B0', variables, filename=output_file, treename="b0", path=self.path)
        ma.variablesToNtuple('gamma', variables, filename=output_file, treename="gamma", path=self.path)

        # J/psi decays
        if self.has_jpsi2ee:
            print("J/psi -> ee")
            ma.variablesToNtuple('e+', variables, filename=output_file, treename="electron", path=self.path)

        elif self.has_jpsi2mumu:
            print("J/psi -> mumu")
            ma.variablesToNtuple('mu+', variables, filename=output_file, treename="muon", path=self.path)

        # eta decays
        if self.has_eta2pipigamma:
            print("eta -> pipigamma")
            ma.variablesToNtuple('pi+', variables, filename=output_file, treename="pion", path=self.path)

        elif self.has_eta2pipipi0:
            print("eta -> pipipi0")
            ma.variablesToNtuple('pi+', variables, filename=output_file, treename="pi", path=self.path)
            ma.variablesToNtuple('pi0', variables, filename=output_file, treename="pi0", path=self.path)
